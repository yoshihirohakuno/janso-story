import { GameState, Tile, PlayerState, Meld, Discard, CallOption, TurnPhase } from './types';
import { setupWall, isDoraTile, getDoraTarget } from './wall';
...
export function initGame(playerNames: string[] = ['Player 1', 'Player 2', 'Player 3', 'Player 4'], playerIsAuto: boolean[] = [false, true, true, true]): GameState {
  const players: PlayerState[] = playerNames.map((name, idx) => ({
    ...
    riichiTurn: null,
    isIppatsu: false,
    seatWind: ['E', 'S', 'W', 'N'][idx] as 'E' | 'S' | 'W' | 'N',
    isFuriten: false,
...
export function startRound(state: GameState): GameState {
  ...
  const windCycle: ('E' | 'S' | 'W' | 'N')[] = ['E', 'S', 'W', 'N'];
...
// Discard a tile
export function discardTile(state: GameState, playerIndex: number, tileId: number, isRiichiDeclaration = false): GameState {
  const next = { ...state };
  const player = next.players[playerIndex];
  
  // Find tile in hand
  const tileIdxInHand = player.hand.findIndex(t => t.id === tileId);
  if (tileIdxInHand === -1) return state; // Invalid tile
...
export function submitCall(state: GameState, playerIndex: number, callType: CallOption['type'], tiles: Tile[] = []): GameState {
  const next = { ...state };
  
  // Find player's call option
  const option = next.activeCalls.find(c => c.playerIndex === playerIndex && c.type === callType);
  if (!option && callType !== 'pass') return state; // Invalid call
...
function calculateDrawPhaseCalls(state: GameState, playerIndex: number, drawnTile: Tile): CallOption[] {
  ...
  if (state.isFirstTurn && player.discards.length === 0 && !player.melds.length) {
    const uniqueYaochu = new Set<string>();
    for (const t of player.hand) {
...
function calculateDiscardPhaseCalls(state: GameState, discarderIndex: number, tile: Tile): CallOption[] {
  const calls: CallOption[] = [];

  for (let p = 0; p < 4; p++) {
    if (p === discarderIndex) continue;
...
function getSuitAndValFromIndex(idx: number): { suit: 'm' | 'p' | 's' | 'z'; value: number } {
  if (idx < 9) return { suit: 'm', value: idx + 1 };
  if (idx < 18) return { suit: 'p', value: idx - 9 + 1 };
  if (idx < 27) return { suit: 's', value: idx - 18 + 1 };
  return { suit: 'z', value: idx - 27 + 1 };
}

// Update Furiten state for all players
function updateFuritenStates(state: GameState) {
  for (let p = 0; p < 4; p++) {
    const player = state.players[p];
    
    // Find all winning tiles (machihai)
    // We check which of the 34 tiles would make the hand shanten = -1
    const winningTileVals: { suit: 'm' | 'p' | 's' | 'z'; value: number }[] = [];
    for (let i = 0; i < 34; i++) {
      const tInfo = getSuitAndValFromIndex(i);
      // Create a dummy tile
      const dummyTile: Tile = { id: 999, suit: tInfo.suit, value: tInfo.value, isRed: false };
...
function handleRonWin(state: GameState, winnerIndex: number): GameState {
  ...
  if (!evalResult) return state; // Should not happen
  ...
  const scoreChanges = redistributeScores(
    winnerIndex,
    discarderIndex,
    pointsData,
    next.dealerIndex,
  ...
  next.scoreChanges = scoreChanges as any; // Convert to array form if needed, let's keep it as array of changes matching player index
  
  const changesArray = [0, 0, 0, 0];
  for (let i = 0; i < 4; i++) changesArray[i] = scoreChanges[i];
  ...
  next.yakuResults = [{
    ...
    basePoints: pointsData.points - next.kyoutaku * 1000 - next.honba * 300,
    kyoutaku: next.kyoutaku,
    honba: next.honba,
    isTsumo: false,
...
function handleAnkan(state: GameState, playerIndex: number, tiles: Tile[]): GameState {
  const next = { ...state };
  const player = next.players[playerIndex];
...
function handleKakan(state: GameState, playerIndex: number, tile: Tile): GameState {
  const next = { ...state };
  const player = next.players[playerIndex];
...
function drawRinshanTile(state: GameState): GameState {
  const next = { ...state };
  const player = next.players[next.activePlayerIndex];
...
function revealNextDora(state: GameState) {
...
function handleExhaustiveDraw(state: GameState): GameState {
  ...
  const tenpais: boolean[] = [false, false, false, false];
  let tenpaiCount = 0;
...
function checkNagashiMangan(state: GameState): number {
  for (let i = 0; i < 4; i++) {
...
function handleNagashiManganWin(state: GameState, winnerIndex: number): GameState {
  const next = { ...state };
  next.turnPhase = 'agari';
...
function checkSpecialDraws(state: GameState): boolean {
  ...
  let totalKans = 0;
  const kanPlayers = new Set<number>();
